...
BEGIN
   ...
   FOR i IN (SELECT wierzcholek from wierzcholki) LOOP
    index_ve := N(i.wierzcholek,i.wierzcholek, -1);
    
    index_sub := nr_indexuf();
    INSERT INTO help VALUES(index_sub, i.wierzcholek);
    
    find_graflet(index_sub, index_ve, index_answear, i.wierzcholek);
   END LOOP;
...
BEGIN
    ...
    FOR i IN ((SELECT wierzcholek2 from krawedzie where wierzcholek1=vi and wierzcholek2!= wierzcholek1) UNION (SELECT wierzcholek1 from krawedzie where wierzcholek2=vi and wierzcholek2!= wierzcholek1))
    LOOP
       
        ilosc := 0;
        SELECT COUNT(*) INTO ilosc from help where index_help = nr and nr_wierzcholka = i.wierzcholek2;
       
        IF ilosc = 0 THEN
          
            nr_wierzcholka_do_wstawienia := i.wierzcholek2;
...
BEGIN
    ...
    SELECT DISTINCT COUNT(*) INTO ab from krawedzie where wierzcholek1 = w1 and wierzcholek2 = w2;
    SELECT DISTINCT COUNT(*) INTO ac from krawedzie where wierzcholek1 = w1 and wierzcholek2 = w3;
    
    SELECT DISTINCT COUNT(*) INTO ba from krawedzie where wierzcholek1 = w2 and wierzcholek2 = w1;
    SELECT DISTINCT COUNT(*) INTO bc from krawedzie where wierzcholek1 = w2 and wierzcholek2 = w3;
    
    SELECT DISTINCT COUNT(*) INTO ca from krawedzie where wierzcholek1 = w3 and wierzcholek2 = w1;
    SELECT DISTINCT COUNT(*) INTO cb from krawedzie where wierzcholek1 = w3 and wierzcholek2 = w2;
